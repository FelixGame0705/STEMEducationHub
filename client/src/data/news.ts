import type { News } from "@shared/schema";

export const newsData: News[] = [
  {
    id: 1,
    title: "Khai trương lớp học AI mới dành cho học sinh THCS",
    slug: "khai-truong-lop-hoc-ai-moi",
    excerpt: "Trung tâm vừa ra mắt chương trình học AI cơ bản dành cho học sinh từ 13-16 tuổi với phương pháp giảng dạy hiện đại và thực hành nhiều.",
    content: "Trung tâm STEM vừa chính thức khai trương lớp học Trí tuệ nhân tạo (AI) dành cho học sinh THCS. Chương trình được thiết kế đặc biệt để giúp các em hiểu về AI một cách dễ dàng và thú vị.\n\nChương trình AI cơ bản bao gồm:\n- Giới thiệu về AI và machine learning\n- Thực hành với Python và các thư viện AI\n- Xây dựng chatbot đơn giản\n- Nhận dạng hình ảnh cơ bản\n- Dự án cuối khóa tự do\n\nVới đội ngũ giảng viên giàu kinh nghiệm và trang thiết bị hiện đại, chúng tôi cam kết mang đến cho học sinh những trải nghiệm học tập tốt nhất về công nghệ AI.",
    image: "https://images.unsplash.com/photo-1497486751825-1233686d5d80?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&h=600",
    category: "TIN TỨC",
    publishedAt: new Date("2024-03-15"),
    featured: true
  },
  {
    id: 2,
    title: "Cuộc thi Robotics toàn quốc - Học viên STEM Center đạt 3 giải",
    slug: "cuoc-thi-robotics-toan-quoc",
    excerpt: "Trong cuộc thi Robotics toàn quốc năm 2024, các học viên của trung tâm đã đạt được những thành tích ấn tượng với 1 giải nhất, 1 giải nhì và 1 giải ba.",
    content: "Cuộc thi Robotics toàn quốc năm 2024 đã khép lại với những thành tích đáng tự hào của các học viên STEM Center. Các em đã giành được 1 giải nhất, 1 giải nhì và 1 giải ba trong các hạng mục khác nhau.\n\nCụ thể:\n- Giải nhất: Nguyễn Minh An (THCS Nguyễn Trãi) - Hạng mục Robot tự động\n- Giải nhì: Vũ Minh Tú (THCS Nguyễn Du) - Hạng mục Robot điều khiển\n- Giải ba: Đội nhóm học sinh lớp Python - Hạng mục Lập trình robot\n\nĐây là kết quả của quá trình học tập và rèn luyện chăm chỉ của các em cùng với sự hướng dẫn tận tình của đội ngũ giáo viên. Chúng tôi tự hào về thành tích này và sẽ tiếp tục đồng hành cùng các em trong hành trình khám phá công nghệ.",
    image: "https://images.unsplash.com/photo-1573164713714-d95e436ab8d6?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&h=600",
    category: "SỰ KIỆN",
    publishedAt: new Date("2024-03-12"),
    featured: true
  },
  {
    id: 3,
    title: "Workshop 'Python cho người mới bắt đầu' - Miễn phí cho phụ huynh",
    slug: "workshop-python-cho-nguoi-moi-bat-dau",
    excerpt: "Một workshop đặc biệt dành cho phụ huynh muốn hiểu rõ hơn về lập trình Python và cách con em học tập tại trung tâm.",
    content: "Trung tâm STEM tổ chức workshop miễn phí dành cho phụ huynh để hiểu rõ hơn về chương trình học Python và cách hỗ trợ con em trong quá trình học tập.\n\nNội dung workshop:\n- Tổng quan về ngôn ngữ lập trình Python\n- Tại sao Python quan trọng cho trẻ em\n- Cách theo dõi tiến độ học tập của con\n- Các dự án thực hành đơn giản\n- Thời gian Q&A với giáo viên\n\nWorkshop diễn ra vào:\n- Thời gian: 14:00 - 16:00, Chủ nhật 24/03/2024\n- Địa điểm: Trung tâm STEM - 123 Đường ABC, Quận 1\n- Đăng ký: Miễn phí, liên hệ hotline để đăng ký chỗ\n\nĐây là cơ hội tuyệt vời để phụ huynh hiểu rõ hơn về chương trình học và cách hỗ trợ con em tại nhà.",
    image: "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&h=600",
    category: "WORKSHOP",
    publishedAt: new Date("2024-03-08"),
    featured: false
  },
  {
    id: 4,
    title: "Cập nhật chương trình Cloud Computing cho học sinh THPT",
    slug: "cap-nhat-chuong-trinh-cloud-computing",
    excerpt: "Trung tâm bổ sung chương trình Cloud Computing nâng cao dành cho học sinh THPT, giúp các em chuẩn bị cho ngành IT.",
    content: "Nhằm đáp ứng nhu cầu học tập về công nghệ cloud ngày càng tăng, STEM Center đã chính thức ra mắt chương trình Cloud Computing nâng cao dành cho học sinh THPT.\n\nChương trình bao gồm:\n- Kiến thức cơ bản về cloud computing\n- Thực hành với AWS và Google Cloud Platform\n- Xây dựng và deploy ứng dụng web\n- Quản lý database trên cloud\n- Bảo mật và monitoring\n- Dự án thực tế cuối khóa\n\nĐây là chương trình tiên tiến giúp các em học sinh chuẩn bị tốt nhất cho việc học đại học ngành IT cũng như nắm bắt xu hướng công nghệ tương lai.",
    image: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&h=600",
    category: "TIN TỨC",
    publishedAt: new Date("2024-03-05"),
    featured: true
  },
  {
    id: 5,
    title: "Chương trình hè đặc biệt: STEM Camp 2024",
    slug: "stem-camp-2024",
    excerpt: "Trại hè STEM 2024 với nhiều hoạt động thú vị, giúp trẻ em khám phá khoa học công nghệ trong môi trường vui nhộn.",
    content: "STEM Center tổ chức chương trình trại hè đặc biệt STEM Camp 2024 với chủ đề 'Khám phá tương lai công nghệ'.\n\nHoạt động chính:\n- Workshop robotics và AI\n- Thí nghiệm khoa học thú vị\n- Thi đấu lập trình\n- Tham quan các công ty công nghệ\n- Hoạt động teambuilding\n- Presentation dự án nhóm\n\nThời gian: 15/06 - 30/06/2024\nĐối tượng: Học sinh từ 8-16 tuổi\nPhí tham gia: 2.500.000 VNĐ (bao gồm ăn trưa, tài liệu, quà tặng)\n\nĐăng ký sớm để nhận ưu đãi 15% và đảm bảo chỗ tham gia.",
    image: "https://images.unsplash.com/photo-1509062522246-3755977927d7?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=800&h=600",
    category: "SỰ KIỆN",
    publishedAt: new Date("2024-03-01"),
    featured: false
  }
];
