    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {contacts?.contacts?.slice(0, 5).map((contact: any) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {news?.news?.filter((n: any) => n.featured).length || 0}
              </div>
              <p className="text-sm text-gray-600">Tin tức nổi bật</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {events?.events?.filter((e: any) => new Date(e.startDate) > new Date()).length || 0}
              </div>
              <p className="text-sm text-gray-600">Sự kiện sắp tới</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">
                {contacts?.contacts?.filter((c: any) => !c.processed).length || 0}
              </div>
              <p className="text-sm text-gray-600">Liên hệ chưa xử lý</p>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">
                {recruitments?.recruitments?.filter((r: any) => new Date(r.deadline) > new Date()).length || 0}
              </div>
