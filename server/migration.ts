import { db } from "./db";
import { adminUsers, programs, students, news, events, recruitments, contacts } from "@shared/schema";
...
async function migrate() {
  ...
  try {
    ...
    for (const recruitment of sampleRecruitments) {
      await db.insert(recruitments).values(recruitment).onDuplicateKeyUpdate({
        set: recruitment
      });
    }
