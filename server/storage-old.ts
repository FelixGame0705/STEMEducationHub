...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    const programsData = [
      {
        ...
        ageRange: "8-12 tuổi",
        objectives:
          "Học lập trình cơ bản, tư duy logic, kỹ năng giải quyết vấn đề",
      ...
      },
      {
        ...
        ageRange: "10-16 tuổi",
        objectives:
          "Nền tảng lập trình, tư duy thuật toán, phát triển game đơn giản",
        image: "/api/placeholder/400/300",
        color: "#8B5CF6",
        createdAt: new Date(),
      },
      {
        ...
        ageRange: "12-16 tuổi",
        objectives:
          "Hiểu biết về AI, xây dựng chatbot đơn giản, phân loại hình ảnh",
        image: "/api/placeholder/400/300",
        color: "#EF4444",
        createdAt: new Date(),
      },
    ...
    const studentsData = [
      {
        ...
        description:
          "An đã xuất sắc lập trình robot Otto thực hiện các động tác phức tạp và giành giải nhất trong cuộc thi Robothon cấp thành phố.",
      ...
      },
      {
        ...
        description:
          "Hà đã tạo ra một hệ thống thông minh giám sát nhiệt độ và độ ẩm sử dụng Microbit và các cảm biến.",
        image: "/api/placeholder/300/400",
        program: "Microbit",
        year: 2024,
        createdAt: new Date(),
      },
      {
        ...
        description:
          "Nam đã tự phát triển một game puzzle bằng Python với giao diện đẹp mắt và gameplay thú vị.",
        image: "/api/placeholder/300/400",
    ...
    const newsData = [
      {
        ...
        excerpt:
          "Trung tâm STEM chính thức ra mắt chương trình học AI cho học sinh từ 12-16 tuổi",
        content:
          "Chương trình học AI mới của chúng tôi sẽ giúp các em học sinh...",
      ...
      },
      {
        ...
        excerpt:
          "3 học viên của trung tâm đã xuất sắc giành giải trong cuộc thi lập trình robot toàn quốc",
      ...
      },
      {
        ...
        excerpt:
          "Tham gia workshop để hiểu thêm về lập trình Python và cách hỗ trợ con em học tập",
        content:
          "Workshop này sẽ cung cấp cho phụ huynh những kiến thức cơ bản...",
        image: "/api/placeholder/600/400",
        category: "WORKSHOP",
    ...
    const eventsData = [
      {
        ...
        description:
          "Triển lãm các sản phẩm robot do học viên tự thiết kế và lập trình",
        image: "/api/placeholder/600/400",
    ...
    const recruitmentsData = [
      {
        ...
        description:
          "Tìm kiếm giảng viên có kinh nghiệm giảng dạy Python cho trẻ em",
        requirements:
          "Tốt nghiệp Công nghệ thông tin, có kinh nghiệm giảng dạy, yêu thích trẻ em",
        benefits:
          "Lương cạnh tranh, môi trường làm việc năng động, cơ hội phát triển",
    ...
    recruitmentsData.forEach((recruitment) =>
  ...
  async getProgramBySlug(slug: string): Promise<Program | undefined> {
    return Array.from(this.programsData.values()).find(
  ...
  async getRecruitmentBySlug(slug: string): Promise<Recruitment | undefined> {
    return Array.from(this.recruitmentsData.values()).find(
      (recruitment) => recruitment.slug === slug,
    );
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async getPrograms(options?: {
    ...
    const query = db.select().from(programs).orderBy(asc(programs.createdAt));

    if (options?.limit) {
      const limitedQuery = query.limit(options.limit);
      if (options.offset) {
        limitedQuery.offset(options.offset);
      }
      const programsResult = await limitedQuery;
      const [{ count }] = await db
        .select({ count: sql<number>`count(*)` })
        .from(programs);
      return { programs: programsResult, total: count };
    }

    const programsResult = await query;
    return { programs: programsResult, total: programsResult.length };
  }
  ...
  async getStudents(options?: {
    ...
    if (options?.limit) {
      ...
      const studentsResult = await limitedQuery;
      ...
      return { students: studentsResult, total: count };
    }

    const studentsResult = await query;
    return { students: studentsResult, total: studentsResult.length };
  }
  ...
  async getNews(options?: {
    ...
    if (options?.limit) {
      ...
      const newsResult = await limitedQuery;
      ...
      return { news: newsResult, total: count };
    }

    const newsResult = await query;
    return { news: newsResult, total: newsResult.length };
  }
  ...
  async getEvents(options?: {
    ...
    if (options?.upcoming) {
      query = query.where(sql`${events.startDate} >= ${new Date()}`);
    }
    ...
    if (options?.limit) {
      ...
      const eventsResult = await limitedQuery;

      let countQuery = db.select({ count: sql<number>`count(*)` }).from(events);
      if (options?.upcoming) {
        countQuery = countQuery.where(
          sql`${events.startDate} >= ${new Date()}`,
        );
      }
      const [{ count }] = await countQuery;

      return { events: eventsResult, total: count };
    }

    const eventsResult = await query;
    return { events: eventsResult, total: eventsResult.length };
  }
  ...
  async getRecruitments(options?: {
    ...
    if (options?.type) {
      query = query.where(eq(recruitments.type, options.type));
    }
    ...
    if (options?.limit) {
      ...
      const recruitmentsResult = await limitedQuery;

      let countQuery = db
        .select({ count: sql<number>`count(*)` })
        .from(recruitments);
      if (options?.type) {
        countQuery = countQuery.where(eq(recruitments.type, options.type));
      }
      const [{ count }] = await countQuery;

      return { recruitments: recruitmentsResult, total: count };
    }

    const recruitmentsResult = await query;
    return {
      recruitments: recruitmentsResult,
      total: recruitmentsResult.length,
    };
  }
  ...
  async getContacts(options?: {
    ...
    if (options?.processed !== undefined) {
      query = query.where(eq(contacts.processed, options.processed));
    }
    ...
    if (options?.limit) {
      ...
      const contactsResult = await limitedQuery;
      ...
      if (options?.processed !== undefined) {
        countQuery = countQuery.where(
          eq(contacts.processed, options.processed),
        );
      }
      const [{ count }] = await countQuery;

      return { contacts: contactsResult, total: count };
    }

    const contactsResult = await query;
    return { contacts: contactsResult, total: contactsResult.length };
  }
